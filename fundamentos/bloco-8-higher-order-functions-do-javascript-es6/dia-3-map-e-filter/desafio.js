...
// const mapCompleto = cities.map((city) => {
//   const nomeEstado = states.find((state) => state.short === city.state);
//   const nomeRegiao = regions.find((region) => region.short === city.region);
//   return {
//     Sigla: `${city.state}`,
//     Cidade: `${city.name}`,
//     Estado: `${nomeEstado.name}`,
//     Regiao: `${nomeRegiao.name}`,
//   };
// });
// console.log(mapCompleto);

function mapCompletoo(){
  const mapCompleto = cities.map((city) => {
    const nomeEstado = states.find((state) => state.short === city.state);
    const nomeRegiao = regions.find((region) => region.short === city.region);
    console.log({
